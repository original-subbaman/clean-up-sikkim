...
export const createTrashDumpPinHandler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    let body;
    ...
    try {
        ...
        const geohash = ngeohash.encode(parseResult.data.lat, parseResult.data.lng, 6);
        ...
        const queryResult = await client.send(new QueryCommand({
            ...
            KeyConditionExpression: "geohash = :geohash AND createdAt >= :sevenDaysAgo",
            ExpressionAttributeValues: {
                ":geohash": geohash,
                ":sevenDaysAgo": sevenDaysAgo,
            },
        }));
        const duplicate = queryResult.Items?.find((item) => item.lat === parseResult.data.lat && item.lng === parseResult.data.lng);
        if (duplicate) {
            return apiResponse(409, {
                message: "A pin with the same lat/lng was created within the last 7 days.",
            });
        }
