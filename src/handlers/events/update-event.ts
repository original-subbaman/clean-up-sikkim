...
export async function updateEvent(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
    try {
        ...
        const fieldsToUpdate = Object.keys(updateData).filter((key) => allowedUpdateFields.includes(key));
        ...
        const expressionAttributeNames = fieldsToUpdate.reduce((acc, field) => {
            acc[`#${field}`] = field;
            return acc;
        }, {} as Record<string, string>);
        const expressionAttributeValues = fieldsToUpdate.reduce((acc, field) => {
