...
export async function getEventsByGeohash(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
    try {
        ...
        const events = await Promise.all(geohashesToQuery.map(async (g) => {
        ...
        const deduplicated = Array.from(new Map(allEvents.map((e) => [e.eventId, e])).values());
        return apiResponse(200, { events: deduplicated });
    }
    catch (error) {
