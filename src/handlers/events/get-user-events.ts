...
export async function getUserEvents(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
    try {
        const eventsParticipantsTable = process.env.EVENT_PARTICIPANTS_TABLE;
        const userId = event.pathParameters?.userId;
        if (!userId || typeof userId !== "string" || userId.trim().length === 0) {
            return apiResponse(400, { message: "Invalid userId path parameter" });
        }
        const userEventsResult = await client.send(new QueryCommand({
