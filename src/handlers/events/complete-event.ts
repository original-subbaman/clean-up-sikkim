...
import { DynamoDBDocumentClient, GetCommand, QueryCommand, QueryCommandOutput, TransactWriteCommand, TransactWriteCommandInput, } from "@aws-sdk/lib-dynamodb";
import { APIGatewayProxyEvent, APIGatewayProxyResult } from "aws-lambda";
import crypto from "crypto";
import { Event } from "../../models/eventSchema";
import { EVENT_STATUS, PIN_STATUS, USER_EVENT_STATUS, } from "../../utils/constants";
 ...
 */
export async function completeEvent(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
    try {
        ...
        if (completedEvent.status === EVENT_STATUS.COMPLETED) {
            return apiResponse(400, { message: "Event is already completed" });
        }
        if (completedEvent.organizedBy !== organizerId) {
            return apiResponse(403, { message: "Unauthorized" });
        }
        const pointsToAward = completedEvent.pointsAwarded || 0;
        const participants = await fetchUsersWhoAttendedEvent(client, eventId);
        if (participants.length === 0) {
            return apiResponse(400, { message: "No participants attended the event" });
        }
        ...
        if (pointsToAward <= 0) {
            return apiResponse(400, { message: "Event has no points configured" });
        }
        const date = new Date().toISOString();
        const transactItems: TransactWriteCommandInput["TransactItems"] = participants.flatMap((userId) => [
...
async function getEvent(client: DynamoDBDocumentClient, eventId: string): Promise<Event | null> {
    const eventsTable = process.env.EVENTS_TABLE;
...
async function fetchUsersWhoAttendedEvent(client: DynamoDBDocumentClient, eventId: string): Promise<string[]> {
    const userIds: string[] = [];
    let lastEvaluatedKey: Record<string, any> | undefined = undefined;
    do {
        const result: QueryCommandOutput = await client.send(new QueryCommand({
