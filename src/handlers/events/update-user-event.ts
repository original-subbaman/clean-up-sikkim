...
export async function updateUserEvent(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
    try {
        ...
        return apiResponse(200, { message: "User event updated successfully" });
    }
    catch (error) {
        console.log("🚀 ~ updateUserEvent ~ error:", error);
        if (error instanceof Error &&
            error.name === "ConditionalCheckFailedException") {
            return apiResponse(404, { message: "Record does not exist" });
        }
